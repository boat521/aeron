...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/aeron-project/aeron>";
    const std::string URL_WEBSITE = "<https://aeron.org>";
...
boost::signals2::connection genesisConnection;
if (chainActive.Tip() == NULL) {
    genesisConnection = uiInterface.NotifyBlockTip.connect(BlockNotifyGenesisWait);
} else {
    fHaveGenesis = true;
}

// Optional blocknotify
boost::signals2::connection blockNotifyConnection;
if (IsArgSet("-blocknotify"))
    blockNotifyConnection = uiInterface.NotifyBlockTip.connect(BlockNotifyCallback);
...
// Wait for genesis block to be processed
{
    ...
    // Safely disconnect using connection objects
    if (genesisConnection.connected()) genesisConnection.disconnect();
}
