...
*Setup instructions for a Gitian build of Aeron Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Aeron
...
- [Building Aeron Core](#building-aeron-core)
...
Clone the git repositories for aeron and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/aeron-project/aeron
...
in the aeron repository under 'Fetch and create inputs' to install sources which require
...
To build Aeron Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the aeron repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aeron/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/aeron-project/aeron
...
For example:
```bash
URL=https://github.com/thrasher-/aeron.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit aeron=${COMMIT} --url aeron=${URL} ../aeron/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aeron=${COMMIT} --url aeron=${URL} ../aeron/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit aeron=${COMMIT} --url aeron=${URL} ../aeron/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the aeron git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../aeron/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/aeron
SIGPATH=/some/root/path/aeron-detached-sigs

./bin/gbuild --url aeron=${BTCPATH},signature=${SIGPATH} ../aeron/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aeron-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aeron-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/aeron-osx-build.assert
```
...
[aeron-project/gitian.sigs.ltc](https://github.com/aeron-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
